import { ComboDetalle, EnvioHeader, Pedido } from "@/app/types";
import { formatearFecha } from "@/app/utils/utils";

export const handleImprimirProductosSeleccionados = (pedidos: Pedido[], seleccionados: number[]) => {

      const productosAImprimir = pedidos.filter(p => seleccionados.includes(p.id));
    ...
    productosAImprimir.forEach(ep => {
      ep.detalles.forEach(det => {
