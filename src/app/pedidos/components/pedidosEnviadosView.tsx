...
import { guardaEstadoPedido, getEnviosPendientes } from "@/app/services/stockService"; // Asegúrate de que estos endpoints existan
...
export default function EnviosPendientesView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const data = await getEnviosPendientes();
      setEnvios(data);
    };
    fetchData();
  }, []);

  const actualizarEstado = async (idPedido: number, nuevoEstado: "entregado" | "cancelado") => {
    try {
      await guardaEstadoPedido({idPedido: idPedido, estado: nuevoEstado});
      setEnvios((prev) =>
        prev.map((envio) => ({
          ...envio,
          envioPedido: envio.envioPedido.map((ep) =>
            ep.pedido.id === idPedido
              ? { ...ep, pedido: { ...ep.pedido, estado: nuevoEstado } }
              : ep
          ),
        }))
      );
  ...
  const handleFinalizarEnvio = async (idEnvio: number) => {
    try {
      const res = await fetch('/api/guardaEstadoPedido', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ idEnvio, nuevoEstado: 'entregado' }),
      });
  ...
  const handleCancelarEnvio = async (idEnvio: number) => {
    try {
      const res = await fetch('/api/guardaEstadoPedido', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ idEnvio, nuevoEstado: 'cancelado' }),
      });
