...
export const handleImprimirProductosVendidos = (pedidosAImprimir: Venta[]) => {
    ...
    <html>
      ...
      <body>
        ${pedidosAImprimir.map(venta => {
          const totalFormateado = Number(venta.total_venta).toLocaleString("es-PY", {
            style: "currency", currency: "PYG"
          });

          return `
            <div class="pedido">
              <h3>Venta #${venta.id} - ${formatearFecha(venta.fecha_venta)}</h3>
              <div class="linea-cliente">
                <strong>Cliente:</strong> ${venta.cliente?.nombre || "No asignado"} | RUC: ${venta.cliente?.ruc || ""} -
              ...
              <table>
                ...
                <tbody>
                  ${venta.salida_stock_general?.salidas.map(d => {
